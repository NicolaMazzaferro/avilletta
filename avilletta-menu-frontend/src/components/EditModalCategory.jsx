...
export default function EditModalCategory({ category, selectedCategoryId, onCloseModal, fetchCategories }) {
    ...
    useEffect(() => {
        ...
        axios.get(`categories/${selectedCategoryId}`)
            .then(({ data }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        await axios.post(`/categories/${selectedCategoryId}`, formData)
            .then(({ data }) => {
                console.log(data.message)
                ...
                setTimeout(() => {
                    ...
                    if (response.data.error.image) {
                        ...
                        setMessagesError(response.data.error.name);
                    } else {
        ...
        const handleDeleteImage = async () => {
            try {
                await axios.delete(`categories/${selectedCategoryId}/delete-image`)
                    .then(({ data }) => {
                        console.log(data);
                        setMessages(data.message);
                        setImage(null);
    ...
    return (
        <>
            ...
            <div className={`fixed inset-0 z-50 flex items-center justify-center overflow-y-auto overflow-x-hidden bg-gray-900 bg-opacity-50 ${selectedCategoryId ? '' : 'hidden'}`} >
                        ...
                        <form onSubmit={handleSubmit} className="p-4 md:p-5">
                            <div className="grid gap-4 mb-4 grid-cols-2">
                                ...
                                {previewImage ? (
                                        ...
                                        <img src={category.image ? `https://api.menu.avillettapizzeria.it/storage/${category.image}` : defaultImage} alt="Anteprima immagine" className="object-cover w-full h-52" />
                                    )}
                                </div> 
